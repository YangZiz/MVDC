...
consistent_loss_up = Consistent_loss_up()
...
up_D_optimizer = optim.Adam(up_D.parameters(),lr=0.00002,betas=(0.5,0.999))
left_D_optimizer = optim.Adam(left_D.parameters(),lr = 0.00002,betas=(0.5,0.999))
right_D_optimizer = optim.Adam(right_D.parameters(),lr = 0.00002,betas=(0.5,0.999))
#D_optimizer = optim.Adam(D.parameters(),lr = 0.00006,betas=(0.5,0.999))
up_LD_optimizer = optim.Adam(up_LD.parameters(),lr=0.00002,betas=(0.5,0.999))
left_LD_optimizer = optim.Adam(left_LD.parameters(),lr = 0.00002,betas=(0.5,0.999))
right_LD_optimizer = optim.Adam(right_LD.parameters(),lr = 0.00002,betas=(0.5,0.999))
...
for epoch in range(now_epoch,epochs):
    ...
    for data in train_loader:
        ...
        up_e4, _ = up_G(up,_z)
        left_e4, _ = left_G(left, _z)
        right_e4, _ = right_G(right, _z)

        e4_list = [up_e4,left_e4,right_e4]
        #print(up_e4.shape)
        stacked_e4 = torch.stack(e4_list,dim=1)
        #print(stacked_e4.shape)
        z,_ = torch.max(stacked_e4,dim=1,keepdim=True)
        z = z.squeeze(1)
        #print(z.shape)

        _, up_G_result = up_G(up, z)
        _, left_G_result = left_G(left, z)
        _, right_G_result = right_G(right, z)
        ...
        up_G_train_loss = -1*up_D_fake_loss-up_LD_fake_loss + L1_lambda * criterionL1(up_G_result, gt_up)+100*up_con_loss #+ Lgro_lambda * criterionL1(Gro_fake, Gro_real)
        ...
        left_con_loss = consistent_loss_left(up_G_result,left_G_result,right_G_result)
        ...
        left_G_train_loss = -1*left_D_fake_loss-left_LD_fake_loss+ L1_lambda * criterionL1(left_G_result, gt_left)+100*left_con_loss#+criterionBCE(D_fake, Variable(torch.ones(D_fake.size()).cuda()))
        ...
        right_con_loss = consistent_loss_right(up_G_result,left_G_result,right_G_result)
        #D_fake = D(up_G_result, left_G_result, right_G_result).squeeze()
        ...
        right_G_train_loss = -1*right_D_fake_loss-right_LD_fake_loss+ L1_lambda * criterionL1(right_G_result,gt_right)+100*right_con_loss#+criterionBCE(D_fake, Variable(torch.ones(D_fake.size()).cuda()))
    ...
    # now_rmse = validate(val_loader,up_G,left_G,right_G)
    #
