...
from mutil_view_model import Mutil_view_Generator,Mutil_Discriminator,Discriminator,Mutil_view_Generator_2,Generator,LocalDiscriminator
...
output_dir = r'/yy/code/pix2pix/output/checkpoint/11_29'
gro_checkpoint = r'/home/jianjian/yangxunyu/yzz2/pix2pix/output/gronet/best_generator.pth'
output_image = r'/yy/code/pix2pix/output/depth_map/11_29'
epochs = 1000
L1_lambda = 100
Lgro_lambda = 50
Lpg_lambda = 50
...
#Mutil_Generator = Mutil_view_Generator_2()
up_G = Generator()
left_G = Generator()
right_G = Generator()
...
up_LD = LocalDiscriminator()
left_LD = LocalDiscriminator()
right_LD = LocalDiscriminator()
...
if (os.path.isfile(os.path.join(output_dir,'best_generator.pth'))):
    up_G.load_state_dict(torch.load(os.path.join(output_dir,'best_generator.pth'))['up_model'])
    left_G.load_state_dict(torch.load(os.path.join(output_dir, 'best_generator.pth'))['left_model'])
    right_G.load_state_dict(torch.load(os.path.join(output_dir, 'best_generator.pth'))['right_model'])
    ...
    best_loss = torch.load(os.path.join(output_dir,'best_generator.pth'))['G_loss']
...
#Mutil_G_optimizer = optim.Adam(Mutil_Generator.parameters(),lr=0.002,betas=(0.5,0.999))
...
up_D_optimizer = optim.Adam(up_D.parameters(),lr=0.000002,betas=(0.5,0.999))
left_D_optimizer = optim.Adam(left_D.parameters(),lr = 0.000002,betas=(0.5,0.999))
right_D_optimizer = optim.Adam(right_D.parameters(),lr = 0.000002,betas=(0.5,0.999))
#D_optimizer = optim.Adam(D.parameters(),lr = 0.00006,betas=(0.5,0.999))
up_LD_optimizer = optim.Adam(up_LD.parameters(),lr=0.000002,betas=(0.5,0.999))
left_LD_optimizer = optim.Adam(left_LD.parameters(),lr = 0.000002,betas=(0.5,0.999))
right_LD_optimizer = optim.Adam(right_LD.parameters(),lr = 0.000002,betas=(0.5,0.999))
...
for epoch in range(now_epoch,epochs):
    ...
    if epoch != now_epoch:
        ...
        up_D_losses.append(up_D_losses_mean)
         #D_losses.append(D_losses_mean)
        left_D_losses.append(left_D_losses_mean)
        right_D_losses.append(right_D_losses_mean)
        up_LD_losses.append(up_LD_losses_mean)
        left_LD_losses.append(left_LD_losses_mean)
        right_LD_losses.append(right_LD_losses_mean)
    ...
    for data in train_loader:
        ...
        gt_up = data['up']['gt']
        rio_up = data['up']['rio']
        up_mask_image = add_mask(inp_up,rio_up)
        ...
        left_mask_image = add_mask(inp_left, rio_left)
        ...
        right_mask_image = add_mask(inp_right, rio_right)
        ...
        #train up_D
        up_D.zero_grad()
        up_D_real = up_D(torch.cat([oppose_up,up_mask_image],1),gt_up).squeeze()
        up_D_real_loss = criterionBCE(up_D_real, Variable(torch.ones(up_D_real.size()).cuda()))

        #up_G_result, _, _ = Mutil_Generator(up, left, right)
        # z = torch.rand(batch_size, 256, 16, 16).cuda()
        # up_e4, _ = up_G(torch.cat([oppose_up, inp_up], 1), z)
        # left_e4, _ = left_G(torch.cat([oppose_left, inp_left], 1), z)
        # right_e4, _ = right_G(torch.cat([oppose_right, inp_right], 1), z)
        ...
        up_D_fake = up_D(torch.cat([oppose_up,up_mask_image],1),up_G_result).squeeze()
        ...
        up_D_real_acu = torch.ge(up_D_real, 0.5).float()
        up_D_fake_acu = torch.le(up_D_fake, 0.5).float()
        up_D_total_acu = torch.mean(torch.cat((up_D_real_acu, up_D_fake_acu), 0))
        if up_D_total_acu <= 0.8:
        ...
        # train left_D
        left_D.zero_grad()
        left_D_real = left_D(torch.cat([oppose_left, left_mask_image], 1), gt_left).squeeze()
        left_D_real_loss = criterionBCE(left_D_real, Variable(torch.ones(left_D_real.size()).cuda()))

        #_, left_G_result, _ = Mutil_Generator(up, left, right)
        # z = torch.rand(batch_size, 256, 16, 16).cuda()
        # up_e4, _ = up_G(torch.cat([oppose_up, inp_up], 1), z)
        # left_e4, _ = left_G(torch.cat([oppose_left, inp_left], 1), z)
        # right_e4, _ = right_G(torch.cat([oppose_right, inp_right], 1), z)
        ...
        left_D_fake = left_D(torch.cat([oppose_left, left_mask_image], 1), left_G_result).squeeze()
        ...
        # train right_D
        right_D.zero_grad()
        right_D_real = right_D(torch.cat([oppose_right, right_mask_image], 1), gt_right).squeeze()
        right_D_real_loss = criterionBCE(right_D_real, Variable(torch.ones(right_D_real.size()).cuda()))

        #_, _, right_G_result = Mutil_Generator(up,left,right)
        # z = torch.rand(batch_size, 256, 16, 16).cuda()
        # up_e4, _ = up_G(torch.cat([oppose_up, inp_up], 1), z)
        # left_e4, _ = left_G(torch.cat([oppose_left, inp_left], 1), z)
        # right_e4, _ = right_G(torch.cat([oppose_right, inp_right], 1), z)
        ...
        right_D_fake = right_D(torch.cat([oppose_right, right_mask_image], 1), right_G_result).squeeze()
        ...
        up_LD_real = up_LD(up_real_local).squeeze()
        up_LD_real_loss = criterionBCE(up_LD_real, Variable(torch.ones(up_LD_real.size()).cuda()))
        up_LD_fake = up_LD(up_fake_local).squeeze()
        up_LD_fake_loss = criterionBCE(up_LD_fake, Variable(torch.zeros(up_LD_fake.size()).cuda()))
        up_LD_train_loss = (up_LD_real_loss + up_LD_fake_loss) * 0.5
        ...
        left_LD_real = left_LD(left_real_local).squeeze()
        left_LD_real_loss = criterionBCE(left_LD_real, Variable(torch.ones(left_LD_real.size()).cuda()))
        left_LD_fake = left_LD(left_fake_local).squeeze()
        left_LD_fake_loss = criterionBCE(left_LD_fake, Variable(torch.zeros(left_LD_fake.size()).cuda()))
        left_LD_train_loss = (left_LD_real_loss + left_LD_fake_loss) * 0.5
        ...
        right_LD_real = right_LD(right_real_local).squeeze()
        right_LD_real_loss = criterionBCE(right_LD_real, Variable(torch.ones(right_LD_real.size()).cuda()))
        right_LD_fake = right_LD(right_fake_local).squeeze()
        right_LD_fake_loss = criterionBCE(right_LD_fake, Variable(torch.zeros(right_LD_fake.size()).cuda()))
        right_LD_train_loss = (right_LD_real_loss + right_LD_fake_loss) * 0.5
        ...
        #
        # mutil_G_train_loss = criterionBCE(D_fake,Variable(torch.ones(D_fake.size()).cuda())) + L1_lambda*(criterionL1(up_G_result,gt_up) + criterionL1(left_G_result,gt_left)+ criterionL1(right_G_result, gt_right))+criterionBCE(up_D_fake,Variable(torch.ones(up_D_fake.size()).cuda()))  + criterionBCE(left_D_fake,Variable(torch.ones(left_D_fake.size()).cuda())) + criterionBCE(right_D_fake,Variable(torch.ones(right_D_fake.size()).cuda()))#+Lpg_lambda*loss_PG#+ Lgro_lambda*criterionL1(Gro_fake,Gro_real)#
        ...
        up_LD_fake = up_LD(up_fake_local_G).squeeze()
        # Gro_fake = Gro(up_G_result)
        # Gro_real = Gro(gt_up)
        ...
        up_G_train_loss = criterionBCE(up_D_fake, Variable(torch.ones(up_D_fake.size()).cuda()))+criterionBCE(up_LD_fake,Variable(torch.ones(up_LD_fake.size()).cuda())) + L1_lambda * criterionL1(up_G_result, gt_up)  # + Lgro_lambda * criterionL1(Gro_fake, Gro_real)
        ...
        left_LD_fake = left_LD(left_fake_local_G).squeeze()

        left_G_train_loss = criterionBCE(left_D_fake,Variable(torch.ones(left_D_fake.size()).cuda())) +criterionBCE(left_LD_fake,Variable(torch.ones(left_LD_fake.size()).cuda()))+ L1_lambda * criterionL1(left_G_result, gt_left)#+criterionBCE(D_fake, Variable(torch.ones(D_fake.size()).cuda()))
        ...
        right_LD_fake = right_LD(right_fake_local_G).squeeze()

        right_D_fake = right_D(torch.cat([oppose_right, right_mask_image], 1), right_G_result).squeeze()

        right_G_train_loss = criterionBCE(right_D_fake, Variable(torch.ones(right_D_fake.size()).cuda())) +criterionBCE(right_LD_fake,Variable(torch.ones(right_LD_fake.size()).cuda()))+ L1_lambda * criterionL1(right_G_result,gt_right)#+criterionBCE(D_fake, Variable(torch.ones(D_fake.size()).cuda()))
    ...
    mutil_G_losses_mean = up_G_losses_mean+0.5*left_G_losses_mean+0.5*right_G_losses_mean
    ...
    if mutil_G_losses_mean<best_loss:
        best_loss = mutil_G_losses_mean
        torch.save({'epoch':epoch,
                    'up_model':up_G.state_dict(),
                    'left_model':left_G.state_dict(),
                    'right_model':right_G.state_dict(),
    ...
    if epoch  % save_interv == 0:
        ...
        save_inter(up_mask_image,left_mask_image,right_mask_image,up_G_result,left_G_result,right_G_result,gt_up,gt_left,gt_right,output_image,epoch)
        ...
        #torch.save(left_LD.state_dict(), os.path.join(output_dir, 'best_left_localdiscriminator.pth'))
        #torch.save(right_LD.state_dict(), os.path.join(output_dir, 'best_right_localdiscriminator.pth'))
    print("epoch-{};,D_up_loss : {:.4}, D_left_loss : {:.4},D_right_loss : {:.4},LD_up_loss : {:.4}, LD_left_loss : {:.4},LD_right_loss : {:.4},mutil_G_loss : {:.4} ".format(epoch+1,up_D_losses_mean,left_D_losses_mean,right_D_losses_mean,up_LD_losses_mean,left_LD_losses_mean,right_LD_losses_mean,mutil_G_losses_mean))
