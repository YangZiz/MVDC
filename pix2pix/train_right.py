...
batch_size = 4
ROOT = r'/yy/data/depth_map/right'
output_dir = r'/yy/code/pix2pix/output/checkpoint/right/11_1'
...
for epoch in range(epochs):
    ...
    for data in train_loader:
        ...
        if G_train_loss<G_best_loss:
            ...
            torch.save(G.state_dict(),os.path.join(output_dir,'best_right_generator.pth'))
            torch.save(D.state_dict(),os.path.join(output_dir,'best_right_discriminator.pth'))
            torch.save(LD.state_dict(), os.path.join(output_dir, 'best_right_localdiscriminator.pth'))
        if (epoch+1)%save_interv == 0:
            torch.save(G.state_dict(), os.path.join(output_dir, '{}_right_generator.pth'.format(epoch+1)))
            torch.save(D.state_dict(), os.path.join(output_dir, '{}_right_discriminator.pth'.format(epoch+1)))
